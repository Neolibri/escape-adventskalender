...
(function () {
  ...
  function renderTag(tag) {
    ...
    if (!istFrei(tag)) {
      view.innerHTML = `
        <button class="zurueck" onclick="location.hash=''">‹ Zurück zur Übersicht</button>
        <div class="gesperrt-box">
          <div class="schloss">🔒</div>
          <div>Dieses Türchen öffnet sich erst am <b>${tag}. Dezember</b>.</div>
        </div>`;
      window.scrollTo(0, 0);
      return;
    }

    const titel = data.titel && data.titel.trim() ? data.titel : ("Tag " + tag);
    ...
    view.innerHTML = `
      <div class="detail">
        ...
        <div class="tag-kopf">
          ...
          <h2>${titel}</h2>
          <div class="datum">${tag}. Dezember</div>
        </div>

        ${data.hinweis1 && data.hinweis1.trim() ? blockHtml("1. Hinweis", data.hinweis1, "💡", "") : ""}
        ${data.hinweis2 && data.hinweis2.trim() ? blockHtml("2. Hinweis", data.hinweis2, "💡", "") : ""}
        ${blockHtml("Lösung", data.loesung, "🗝️", "loesung")}
      </div>`;
